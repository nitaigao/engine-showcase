...
#ifdef XML_DTD
  paramEntityParsing = (enum XML_ParamEntityParsing) oldParamEntityParsing;
...
#ifdef XML_CONTEXT_BYTES
      if (keep1 < bufferPtr - buffer) {
        int offset = (bufferPtr - buffer) - keep1;
        memmove(buffer, &buffer[offset], bufferEnd - bufferPtr + keep1);
        bufferEnd -= offset;
        bufferPtr -= offset;
      }
#else
...
XML_ErrorString(enum XML_Error code)
{
  ...
  if ((int)code > 0 && (int)code < (int) (sizeof(message)/sizeof(message[0])))
    return message[code];
  return NULL;
}
    ...
    case XML_ROLE_GROUP_OPEN:
      if (prologState.level >= groupSize) {
        if (groupSize) {
          ...
          if (dtd.scaffIndex)
          {
            int *temp2 = (int*) REALLOC(dtd.scaffIndex, groupSize * sizeof(int));
            if (temp2 == NULL)
              return XML_ERROR_NO_MEMORY;
