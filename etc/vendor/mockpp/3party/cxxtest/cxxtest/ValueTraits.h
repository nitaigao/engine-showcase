...
CXXTEST_NS_START
    ...
    //
#   define TS_AS_STRING(x) CXXTEST_NS::traits(x).asString()

    //
    // Char representation of a digit
    //
    CXXTEST_API_DECL(char) digitToChar( unsigned digit );

    //
    // Convert byte value to hex digits
    // Returns pointer to internal buffer
    //
    CXXTEST_API_DECL(const char *) byteToHex( unsigned char byte );

    //
    // Convert byte values to string
    // Returns one past the copied data
    //
    CXXTEST_API_DECL(char *) bytesToString( const unsigned char *bytes, unsigned numBytes, unsigned maxBytes, char *s );
    ...
    //
    CXXTEST_API_DECL(char *) copyString( char *dst, const char *src );

    //
    // Compare two strings.
    // Remember -- we can't use the standard library!
    //
    CXXTEST_API_DECL(bool) stringsEqual( const char *s1, const char *s2 );
    ...
    //
    CXXTEST_API_DECL(char *) charToString( unsigned long c, char *s );

    //
    // Prevent problems with negative (signed char)s
    //
    CXXTEST_API_DECL(char *) charToString( char c, char *s );
...
#   endif // _CXXTEST_LONGLONG
    ...
    public:
        ...
        {
            ( requiredDigitsOnLeft( t ) > (unsigned) MAX_DIGITS_ON_LEFT ) ?
                hugeNumber( t ) :
                normalNumber( t );
        }
    ...
    private:
        ...
        static CXXTEST_API_DECL(unsigned) requiredDigitsOnLeft( double t );
        CXXTEST_API_DECL(char *) doNegative( double &t );
        CXXTEST_API_DECL(void) hugeNumber( double t );
        CXXTEST_API_DECL(void) normalNumber( double t );
        CXXTEST_API_DECL(char *) doubleToString( double t, char *s, unsigned skip = 0, unsigned max = (unsigned)-1 );
    };
