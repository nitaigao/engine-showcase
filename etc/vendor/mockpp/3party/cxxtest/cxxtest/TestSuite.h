...
CXXTEST_NS_START
    class CXXTEST_API_DECL0 TestSuite
    {
    public:
        virtual ~TestSuite();
        virtual void setUp();
    ...
    class AbortTest {};
    CXXTEST_API_DECL(void) doAbortTest();
#   define TS_ABORT() CXXTEST_NS::doAbortTest()

    CXXTEST_API_DECL(bool) abortTestOnFail();
    CXXTEST_API_DECL(void) setAbortTestOnFail( bool value = CXXTEST_DEFAULT_ABORT );

    CXXTEST_API_DECL(unsigned) maxDumpSize();
    CXXTEST_API_DECL(void) setMaxDumpSize( unsigned value = CXXTEST_MAX_DUMP_SIZE );

    CXXTEST_API_DECL(void) doTrace( const char *file, unsigned line, const char *message );
    CXXTEST_API_DECL(void) doWarn( const char *file, unsigned line, const char *message );
    CXXTEST_API_DECL(void) doFailTest( const char *file, unsigned line, const char *message );
    CXXTEST_API_DECL(void) doFailAssert( const char *file, unsigned line, const char *expression, const char *message );
    ...
    CXXTEST_API_DECL(void)
		 doAssertSameData( const char *file, unsigned line,
    ...
    CXXTEST_API_DECL(void) 
		 doFailAssertThrows( const char *file, unsigned line,
                             const char *expr, const char *type,
                             bool otherThrown,
                             const char *message );

    CXXTEST_API_DECL(void) 
		 doFailAssertThrowsNot( const char *file, unsigned line,
                                const char *expression, const char *message );

#   ifdef _CXXTEST_HAVE_EH
#       define _TS_TRY try
#       define _TS_CATCH_TYPE(t, b) catch t b
#       define _TS_CATCH_ABORT(b) _TS_CATCH_TYPE( (const CXXTEST_NS::AbortTest &), b )
#       define _TS_LAST_CATCH(b) _TS_CATCH_TYPE( (...), b )
#       define _TSM_LAST_CATCH(f,l,m) _TS_LAST_CATCH( { (CXXTEST_NS::tracker()).failedTest(f,l,m); } )
#       ifdef _CXXTEST_HAVE_STD
...
#   endif // _CXXTEST_HAVE_EH
    ...
    // TS_TRACE
#   define _TS_TRACE(f,l,e) CXXTEST_NS::doTrace( (f), (l), TS_AS_STRING(e) )
#   define TS_TRACE(e) _TS_TRACE( __FILE__, __LINE__, e )

    // TS_WARN
#   define _TS_WARN(f,l,e) CXXTEST_NS::doWarn( (f), (l), TS_AS_STRING(e) )
#   define TS_WARN(e) _TS_WARN( __FILE__, __LINE__, e )

    // TS_FAIL
#   define _TS_FAIL(f,l,e) CXXTEST_NS::doFailTest( (f), (l), TS_AS_STRING(e) )
#   define TS_FAIL(e) _TS_FAIL( __FILE__, __LINE__, e )

    // TS_ASSERT
#   define ___ETS_ASSERT(f,l,e,m) { if ( !(e) ) CXXTEST_NS::doFailAssert( (f), (l), #e, (m) ); }
...
#   define TSM_ASSERT(m,e) _TSM_ASSERT(__FILE__,__LINE__,m,e)
    ...
    // TS_ASSERT_EQUALS
#   define ___ETS_ASSERT_EQUALS(f,l,x,y,m) CXXTEST_NS::doAssertEquals( (f), (l), #x, (x), #y, (y), (m) )
...
#   define TSM_ASSERT_EQUALS(m,x,y) _TSM_ASSERT_EQUALS(__FILE__,__LINE__,m,x,y)
    ...
    // TS_ASSERT_SAME_DATA
#   define ___ETS_ASSERT_SAME_DATA(f,l,x,y,s,m) CXXTEST_NS::doAssertSameData( (f), (l), #x, (x), #y, (y), #s, (s), (m) )
...
#   define TSM_ASSERT_SAME_DATA(m,x,y,s) _TSM_ASSERT_SAME_DATA(__FILE__,__LINE__,m,x,y,s)
    ...
    // TS_ASSERT_DIFFERS
#   define ___ETS_ASSERT_DIFFERS(f,l,x,y,m) CXXTEST_NS::doAssertDiffers( (f), (l), #x, (x), #y, (y), (m) )
...
#   define TSM_ASSERT_DIFFERS(m,x,y) _TSM_ASSERT_DIFFERS(__FILE__,__LINE__,m,x,y)
    ...
    // TS_ASSERT_LESS_THAN
#   define ___ETS_ASSERT_LESS_THAN(f,l,x,y,m) CXXTEST_NS::doAssertLessThan( (f), (l), #x, (x), #y, (y), (m) )
...
#   define ___ETS_ASSERT_LESS_THAN_EQUALS(f,l,x,y,m) \
        CXXTEST_NS::doAssertLessThanEquals( (f), (l), #x, (x), #y, (y), (m) )
...
#   define ___ETS_ASSERT_PREDICATE(f,l,p,x,m) \
        CXXTEST_NS::doAssertPredicate( (f), (l), #p, p(), #x, (x), (m) )
...
#   define ___ETS_ASSERT_RELATION(f,l,r,x,y,m) \
        CXXTEST_NS::doAssertRelation( (f), (l), #r, r(), #x, (x), #y, (y), (m) )
...
#   define TSM_ASSERT_RELATION(m,r,x,y) _TSM_ASSERT_RELATION(__FILE__,__LINE__,m,r,x,y)
    ...
    // TS_ASSERT_DELTA
#   define ___ETS_ASSERT_DELTA(f,l,x,y,d,m) CXXTEST_NS::doAssertDelta( (f), (l), #x, (x), #y, (y), #d, (d), (m) )
...
#   define ___TS_ASSERT_THROWS(f,l,e,t,m) { \
            ...
            if ( !_ts_threw_expected ) { CXXTEST_NS::doFailAssertThrows( (f), (l), #e, #t, _ts_threw_else, (m) ); } }
...
#   define ___TS_ASSERT_THROWS_ANYTHING(f,l,e,m) { \
            ...
            if ( !_ts_threw ) { CXXTEST_NS::doFailAssertThrows( (f), (l), #e, "...", false, (m) ); } }
...
#   define ___TS_ASSERT_THROWS_NOTHING(f,l,e,m) { \
            _TS_TRY { e; } \
            _TS_CATCH_ABORT( { throw; } ) \
            _TS_LAST_CATCH( { CXXTEST_NS::doFailAssertThrowsNot( (f), (l), #e, (m) ); } ) }
