...
hkDemo::Result InteractionCharacterRbDemo::stepDemo()
{
	...
	// Do character
	{
		...
		// Calculate and set current acceleration for mass factor modifier
		{
			...
			hkVector4 currentVel; currentVel = m_characterRigidBody->getRigidBody()->getLinearVelocity();

			hkVector4 currentAcc; currentAcc.setSub4(output.m_velocity, currentVel);
			currentAcc.mul4(1/m_timestep);
