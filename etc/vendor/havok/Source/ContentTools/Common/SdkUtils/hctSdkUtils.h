...
class hctSdkUtils
{
	public:
		...
		static const hkArray<hctModelerNodeIdToClassMap>& getNodeIdToClassMap() {return m_modelerNodeIdToClassMap;}
			...
			/// Returns the number of all registered classes excluding any base classes (i.e. ones that have defined with m_id == -1).
		static int getNumUiClasses();

			/// Returns the i'th registered class from the table while excluding any base classes (i.e. ones that have defined with m_id == -1).
