...
namespace AI
{
	NavigationPolygon::PolyEdgeList NavigationPolygon::GetEdges( ) const
	{
		...
		{
			PolyEdge edge( m_vertices[ i - 1 ], m_vertices[ i ] );
			edges.push_back( edge );
		}

		PolyEdge lastEdge( m_vertices[ m_vertices.size( ) - 1 ], m_vertices[ 0 ] );
	...
	{
		return
			this->SameSide( point, m_vertices[ 0 ], m_vertices[ 1 ], m_vertices[ 2 ] ) &&
			this->SameSide( point, m_vertices[ 1 ], m_vertices[ 0 ], m_vertices[ 2 ] ) &&
			this->SameSide( point, m_vertices[ 2 ], m_vertices[ 0 ], m_vertices[ 1 ] );
	}
	...
	void NavigationPolygon::AddNeighbour( NavigationPolygon* polygon )
	{
		...
		for ( NavigationPolygon::PolyEdgeList::iterator existingEdge = existingEdges.begin( ); existingEdge != existingEdges.end( ); ++existingEdge )
		{
			...
			for ( NavigationPolygon::PolyEdgeList::iterator newEdge = newEdges.begin( ); newEdge != newEdges.end( ); ++newEdge )
			{
				...
				{
					//polygon->m_neighbours.push_back( this );
					//polygon->m_edgeLinks.push_back( *newEdge );
