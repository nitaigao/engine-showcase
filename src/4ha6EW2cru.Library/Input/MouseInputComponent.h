...
*  @file   MouseInputComponent.h
*  @date   2009/05/09
*/
#ifndef __FIRSTPERSONMOUSEINPUTCOMPONENT_H
#define __FIRSTPERSONMOUSEINPUTCOMPONENT_H

#include <ois/OIS.h>

#include "IMouseInputComponent.hpp"

namespace Input
{
	/*!
	 *  A First Person Mouse Input Component 
	 */
	class MouseInputComponent : public IMouseInputComponent
	{
	...
	public:
		...
		MouseInputComponent( const std::string& name, OIS::Mouse* mouse )
			: _name( name )
			, _id( 0 )
			, _mouse( mouse )
		{
	...
	private:
		...
		MouseInputComponent( const MouseInputComponent & copy ) { };
		MouseInputComponent & operator = ( const MouseInputComponent & copy ) { return *this; };
